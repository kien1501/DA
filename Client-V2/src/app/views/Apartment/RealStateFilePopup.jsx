...
class RealStateFilePopup extends Component {
  state = {
    name: "",
    code: "",
    description: "",
  ...
  fileUpload(file) {
    const url = ConstantList.API_ENPOINT + "/api/fileUpload/realState/realStateDocument/uploadattachment";
  ...
  handleViewDocument = index => {
    ...
    const url = ConstantList.API_ENPOINT + "/api/fileDownload/document/" + file.file.id;
  ...
  render() {
    let { open, handleClose,handleSelect, handleOKEditClose, t, i18n } = this.props;
          ...
          <DialogContent>
            <Grid className="mb-10" container spacing={1}>
              ...
              <Grid item md={9} sm={12} xs={12}>
                <TextValidator
                  className="w-100"
                  label={<span><span style={{ color: "red" }}>*</span>{t('file.name')}</span>}
            ...
            <div className="mt-15">
              ...
              <Card className="mb-24" elevation={2}>
                ...
                {files.map((item, index) => {
                  let { file, uploading, error, progress, isEditRealStateDocument, contentSize } = item;
                  return (
                    <div className="px-16 py-16" key={file.name}>
                      ...
                      >
                        ...
                        <Grid item lg={4} md={4} sm={12} xs={12}>
                          <div className="flex">
                            {!isEditRealStateDocument && (
                              <IconButton title={t('general.upload')} onClick={() => this.uploadSingleFile(index)}>
                                <Icon color="primary">cloud_upload</Icon>
                              </IconButton>
                            )}
                            {isEditRealStateDocument && (<IconButton title={t('general.viewDocument')} onClick={() => this.handleViewDocument(index)}>
                              <Icon color="primary">visibility</Icon>
                            </IconButton>
                            )}

                            {isEditRealStateDocument && (<IconButton title={t('general.downloadDocument')} onClick={() => this.handleDownloadDocument(index)}>
...
export default RealStateFilePopup;
