...
import {searchByPage} from "./BuildingService";
import { Link } from "react-router-dom";
import SearchIcon from '@material-ui/icons/Search';
import Autocomplete from "@material-ui/lab/Autocomplete";
import {searchByPage as getListProject} from '../Project/ProjectService'
...
class SelectParentPopup extends React.Component {
  ...
  updatePageData = () => {
    ...
    searchObject.adminUnitId = this.state.administrative ? this.state.administrative.id :""
    searchObject.pageIndex = this.state.page + 1;
    searchObject.pageSize = this.state.rowsPerPage;
    getListProject(searchObject).then(({ data }) => {
    ...
    return (
      <Dialog onClose={handleClose} open={open} PaperComponent={PaperComponent} maxWidth={'md'} fullWidth>
        <DialogTitle style={{ cursor: 'move' }} id="draggable-dialog-title">
          <span className="mb-20">{t("title.project")}</span>
        </DialogTitle>
        <DialogContent>
